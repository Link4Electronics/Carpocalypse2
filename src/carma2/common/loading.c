...
void C2_HOOK_FASTCALL PackFileRerevertTiffLoading(void) {
    ...
    if (gDisableTiffConversionStackPos != 0) { // or > 0 if it is unsigned
        gDisableTiffConversion = gDisableTiffConversionStack[gDisableTiffConversionStackPos];
    }
}
...
void C2_HOOK_FASTCALL LoadGeneralParameters(void) {
    ...
    GetAString(gTempFile, gDefaultCar);
    GetAString(gTempFile, gBasic_car_name);

    gOpponent_nastyness_frigger = 1.f;
    ...
    gPickup_respawn_max_extra_time_ms = 100 * GetAnInt(gTempFile);
...
void C2_HOOK_FASTCALL LoadHeadupImages(void) {
    ...
    for (i = 0; i < CARPOCALYPSE2_ASIZE(gHeadup_image_info); i++) {
        ...
        if (gHeadup_image_info[i].avail == eNet_or_otherwise
                ...
                || (gHeadup_image_info[i].avail == eNet_only && gNet_mode != eNet_mode_none)) {
            pixmap = LoadPixelmap(gHeadup_image_info[i].name);
            BRPM_convert(pixmap, gBack_screen->type);
        } else {
            pixmap = NULL;
        }
        gHeadup_images[i] = pixmap;
    }
}
...
int C2_HOOK_FASTCALL TestForOriginalCarmaCDinDrive(void) {
    ...
    PathCat(paths_txt, gApplication_path, "PATHS.TXT");
